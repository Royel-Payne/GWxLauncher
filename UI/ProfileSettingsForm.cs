...
namespace GWxLauncher.UI
{
    public partial class ProfileSettingsForm : Form
    {
        ...
        {
            if (_generalTab == null || _loginTab == null || _modsTab == null || _pnlButtonBar == null)
                return;

            // ===== GENERAL TAB =====
            this.Controls.Remove(txtProfileName);
            this.Controls.Remove(label1);
            this.Controls.Remove(txtExecutablePath);
            this.Controls.Remove(label2);
            this.Controls.Remove(btnBrowseExe);
            this.Controls.Remove(txtLaunchArgs);
            this.Controls.Remove(lblLaunchArgs);
            this.Controls.Remove(txtGw1WindowTitleLabel);
            this.Controls.Remove(lblGw1WindowTitleLabel);
            this.Controls.Remove(chkGw1Multiclient);

            _generalTab.Controls.AddRange(new Control[]
            {
                label1, txtProfileName,
                label2, txtExecutablePath, btnBrowseExe,
                lblLaunchArgs, txtLaunchArgs,
                lblGw1WindowTitleLabel, txtGw1WindowTitleLabel,
                chkGw1Multiclient
            });

            RepositionGeneralTabControls();

            // ===== LOGIN TAB ===== (NEW APPROACH)
            var gw1LoginControls = grpGw1Login.Controls.Cast<Control>().ToArray();
            var gw2LoginControls = grpGw2Login.Controls.Cast<Control>().ToArray();

            foreach (var ctrl in gw1LoginControls)
                grpGw1Login.Controls.Remove(ctrl);

            foreach (var ctrl in gw2LoginControls)
                grpGw2Login.Controls.Remove(ctrl);

            this.Controls.Remove(grpGw1Login);
            this.Controls.Remove(grpGw2Login);

            // Call the new ArrangeControls method
            _loginTab.ArrangeControls(gw1LoginControls, gw2LoginControls);

            // ===== MODS TAB ===== (NEW APPROACH)
        ...
        private void RepositionGeneralTabControls()
        {
            int leftCol1 = 10;
            int leftCol2 = 110;
            int rowHeight = 35;
            ...
            // Row 1: Display Name
            label1.Location = new Point(leftCol1, 10);
            txtProfileName.Location = new Point(leftCol2, 10);
            txtProfileName.Width = 200;

            // Row 2: Game EXE Path
            label2.Location = new Point(leftCol1, 10 + rowHeight);
            txtExecutablePath.Location = new Point(leftCol2, 10 + rowHeight);
            txtExecutablePath.Width = 280;
            btnBrowseExe.Location = new Point(leftCol2 + 290, 10 + rowHeight);

            // Row 3: Arguments
            lblLaunchArgs.Location = new Point(leftCol1, 10 + rowHeight * 2);
            txtLaunchArgs.Location = new Point(leftCol2, 10 + rowHeight * 2);
            txtLaunchArgs.Width = 380;

            // Row 4: Titlebar Label (GW1 only)
            lblGw1WindowTitleLabel.Location = new Point(leftCol1, 10 + rowHeight * 3);
            txtGw1WindowTitleLabel.Location = new Point(leftCol2, 10 + rowHeight * 3);
            txtGw1WindowTitleLabel.Width = 200;

            // Row 5: Multiclient checkbox
            chkGw1Multiclient.Location = new Point(leftCol1, 10 + rowHeight * 4);
        }
        ...
        private void lstTabs_SelectedIndexChanged(object? sender, EventArgs e)
        {
            ...
            switch (_lstTabs.SelectedIndex)
            {
                ...
                case 1: // Login
                    ...
                    _loginTab.UpdateForGameType(_profile.GameType);
                    _loginTab.Focus();
                    break;
